...
# We do not use it here (we reuse the nuget.exe of the CK-DB solution), but we create
# the folder anyway.
...
$nugetExe = Join-Path $toolsDir "../../CodeCakeBuilder/Tools/nuget.exe"
if (!(Test-Path $nugetExe)) {
	Throw "NuGet.exe must be is CK-DB Solution (in CodeCakeBuilder/Tools directory)."
}
