...
namespace CK.DB.Res.MCResHtml.Tests
{
    ...
    public class MCResHtmlTests
    {
        ...
        {
            p.Database.AssertScalarEquals( expectedValue, "select Value from CK.vMCResHtml where ResId=@0 and XLCID = @1", resId, lcid )
                      .AssertScalarEquals( expectedLCID, "select LCID from CK.vMCResHtml where ResId=@0 and XLCID = @1", resId, lcid );
        }
        ...
        public void destroying_the_resource_destroys_the_string_values()
        {
            ...
            using( var ctx = new SqlStandardCallContext() )
            {
                ...
                p.Database.AssertEmptyReader( "select Value from CK.vMCResHtml where ResId=@0", bothId );
