...
namespace CK.DB.Res.MCResString.Tests
{
    ...
    public class MCResStringTests
    {
        ...
        static void AssumeFallbackTestEnglishAndFrenchResources( Package p, SqlStandardCallContext ctx, out int noValuesId, out int enId, out int frId, out int bothId )
        {
            ...
            p.MCResStringTable.SetString( ctx, enId, 9, "Only in English." );
            frId = p.ResTable.Create( ctx );
            p.MCResStringTable.SetString( ctx, frId, 12, "Seulement en Français." );
            bothId = p.ResTable.Create( ctx );
            p.MCResStringTable.SetString( ctx, bothId, 9, "English (and French)." );
            p.MCResStringTable.SetString( ctx, bothId, 12, "Français (et Anglais)." );
        }

        static void CheckString( Package p, int resId, int lcid, object expectedValue, object expectedLCID )
        {
            p.Database.AssertScalarEquals( expectedValue, "select Value from CK.vMCResString where ResId=@0 and XLCID = @1", resId, lcid )
                      .AssertScalarEquals( expectedLCID, "select LCID from CK.vMCResString where ResId=@0 and XLCID = @1", resId, lcid );
        }
        ...
        public void setting_and_clearing_string_values_can_use_XLCID()
        {
            ...
            using( var ctx = new SqlStandardCallContext() )
            {
                ...
                p.MCResStringTable.SetString( ctx, resId, xlcid1, "الأزمة في مصر" );
                ...
                p.MCResStringTable.SetString( ctx, resId, xlcid10, "¡Hola! (España)" );
                CheckString( p, resId, 1, "الأزمة في مصر", 1 );
                ...
                p.MCResStringTable.SetString( ctx, resId, 12, "Liberté!" );
                CheckString( p, resId, 1, "الأزمة في مصر", 1 );
                ...
                p.MCResStringTable.SetString( ctx, resId, xlcid1, null );
                ...
                p.MCResStringTable.SetString( ctx, resId, xlcid10, null );
                CheckString( p, resId, 1, "Liberté!", 12 );
                CheckString( p, resId, 9, "Liberté!", 12 );
        ...
        public void destroying_the_resource_destroys_the_string_values()
        {
            ...
            using( var ctx = new SqlStandardCallContext() )
            {
                ...
                p.Database.AssertEmptyReader( "select Value from CK.vMCResString where ResId=@0", bothId );

                Assert.DoesNotThrow( () => p.MCResStringTable.SetString( ctx, bothId, 9, null ) );
            }
