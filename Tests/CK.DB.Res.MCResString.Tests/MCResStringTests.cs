...
namespace CK.DB.Res.MCResString.Tests
{
    ...
    public class MCResStringTests
    {
        ...
        {
            p.Database.ExecuteScalar( "select Value from CK.vMCResString where ResId=@0 and XLCID = @1", resId, lcid )
                    .Should().Be( expectedValue );
            p.Database.ExecuteScalar( "select LCID from CK.vMCResString where ResId=@0 and XLCID = @1", resId, lcid )
